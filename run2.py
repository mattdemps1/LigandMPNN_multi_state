...
from model_utils2 import ProteinMPNN
    ...
    for pdb in pdb_paths:
        ...
        with torch.no_grad():
            ...
            if args.save_logits:
                ...
                # 2. Average the collected logits after the loop
                if logits_list:
                    # Stack all collected logits into one big tensor
                    stacked_logits = torch.cat(logits_list, 0)
                    # Calculate the average across all the samples (dimension 0)
                    averaged_logits = torch.mean(stacked_logits, dim=0, keepdim=True)

                    # 3. Create path and save the final averaged logits file
